...
<div class="page-header">
    <div class="page-header-row">
        <div>
            <h1 class="page-title">Edit Hospital: {{ $hospital->name }}</h1>
            <p class="page-description">Update your hospital's information.</p>
        </div>
    </div>
</div>
...
<div class="card" style="max-width: 800px;">
    ...
    <form action="{{ route('hospital-owner.hospitals.update', $hospital) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            <div class="form-group">
                <label for="name" class="form-label">Hospital Name</label>
                <input type="text" id="name" name="name" class="form-input @error('name') is-invalid @enderror" value="{{ old('name', $hospital->name) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select id="country_id" name="country_id" class="form-input @error('country_id') is-invalid @enderror" required>
                        ...
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}" {{ old('country_id', $hospital->country_id) == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="thana_id" name="thana_id" class="form-input @error('thana_id') is-invalid @enderror" required>
                        ...
                        @foreach($thanas as $thana)
                            <option value="{{ $thana->id }}" {{ old('thana_id', $hospital->thana_id) == $thana->id ? 'selected' : '' }}>{{ $thana->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <textarea id="address" name="address" class="form-input @error('address') is-invalid @enderror" rows="3" required>{{ old('address', $hospital->address) }}</textarea>
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="phone" name="phone" class="form-input @error('phone') is-invalid @enderror" value="{{ old('phone', $hospital->phone) }}">
                    @error('phone') <span class="form-error">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label for="latitude" class="form-label">Latitude (optional)</label>
                    <input type="text" id="latitude" name="latitude" class="form-input @error('latitude') is-invalid @enderror" value="{{ old('latitude', $hospital->latitude) }}">
                </div>
            </div>

            <div class="form-group">
                <label for="description" class="form-label">Description</label>
                <textarea id="description" name="description" class="form-input @error('description') is-invalid @enderror" rows="4">{{ old('description', $hospital->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select id="doctors" name="doctors[]" class="form-input" multiple style="height: 120px;">
                    @foreach($doctors as $doctor)
                        <option value="{{ $doctor->id }}" {{ in_array($doctor->id, old('doctors', $hospital->doctors->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $doctor->name }} ({{ $doctor->specialization ?? 'General' }})</option>
                    @endforeach
                </select>
...
window.onload = () => {
    let featureCount = 0;
    @foreach($hospital->features as $f)
        addFeature("{!! addslashes($f->name) !!}");
        featureCount++;
    @endforeach
    ...
    @foreach($hospital->services as $s)
        addService("{!! addslashes($s->name) !!}", "{{ $s->price }}");
        serviceCount++;
    @endforeach
