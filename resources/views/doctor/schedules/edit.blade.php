...
<div class="page-header">
    <div class="page-header-row">
        <div>
            ...
            <p class="page-description">Update your availability slot.</p>
        </div>
    </div>
</div>
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <select id="hospital_id" name="hospital_id" class="form-input @error('hospital_id') is-invalid @enderror" required>
                    @foreach($hospitals as $hospital)
                        <option value="{{ $hospital->id }}" {{ old('hospital_id', $schedule->hospital_id) == $hospital->id ? 'selected' : '' }}>{{ $hospital->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid-2">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" id="visit_fee" name="visit_fee" class="form-input @error('visit_fee') is-invalid @enderror" value="{{ old('visit_fee', $schedule->visit_fee) }}" required>
            ...
            <div class="grid-2">
                <div class="form-group">
                    ...
                    <input type="time" id="start_time" name="start_time" class="form-input @error('start_time') is-invalid @enderror" value="{{ old('start_time', $schedule->start_time) }}" required>
                    @error('start_time') <span class="form-error">{{ $message }}</span> @enderror
                </div>

                <div class="form-group">
                    <label for="end_time" class="form-label">End Time</label>
                    <input type="time" id="end_time" name="end_time" class="form-input @error('end_time') is-invalid @enderror" value="{{ old('end_time', $schedule->end_time) }}" required>
