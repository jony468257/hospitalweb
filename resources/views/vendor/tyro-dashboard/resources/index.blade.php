...
<!-- Table -->
<div class="card" style="box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); border: none;">
    @if($items->count())
        ...
        <table class="table">
            <thead>
                <tr style="background: var(--muted); border-bottom: 2px solid var(--border);">
                    @foreach($config['fields'] as $key => $field)
                    @if(!($field['hide_in_index'] ?? false))
                    <th data-col="{{ $key }}" style="text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.05em; font-weight: 700; padding: 1rem 1.25rem;">{{ $field['label'] }}</th>
                    @endif
                    @endforeach
                    <th style="text-align: right; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.05em; font-weight: 700; padding: 1rem 1.25rem;">Actions</th>
                </tr>
            </thead>
            <tbody>
